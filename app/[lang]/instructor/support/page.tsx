...
export default async function InstructorSupportPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  try {
    ...
    console.error("[INSTRUCTOR_SUPPORT_LIST] Failed to list tickets:", err)
    raw = []
  }
    ...
    <div className="container py-8 space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {isAr ? `مرحباً ${user.name}، نحن هنا لمساعدتك كمدرب` : `Hi ${user.name}, we're here to help as an instructor`}
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {isAr
              ? "عرض جميع تذاكر الدعم التي قمت بإنشائها كمدرب"
              : "View all support tickets you have created as an instructor"}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {tickets.length === 0 ? (
            ...
            <div className="space-y-3">
              {tickets.map((ticket) => (
                <Link
                  key={ticket.id}
                  href={`/${lang}/instructor/support/${ticket.id}`}
